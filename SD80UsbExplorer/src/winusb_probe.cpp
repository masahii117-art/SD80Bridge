...
// Windows.Devices.Midi / C++/WinRT input probe.
// 0 = disabled (default).
// 1 = enumerate/open a Windows MIDI input device and receive MIDI.
#ifndef SD80BRIDGE_WINRT_MIDI_TEST
#define SD80BRIDGE_WINRT_MIDI_TEST 0
#endif

// First device shown by the probe. Leave at 0 until the device list is known.
#ifndef SD80BRIDGE_WINRT_MIDI_DEVICE_INDEX
#define SD80BRIDGE_WINRT_MIDI_DEVICE_INDEX 0
#endif
    ...
    WINUSB_INTERFACE_HANDLE winusbHandle)
{
    ...
    std::wcout
        << L"USB-MIDI INPUT RUNNING v10.16"
        << std::endl;
...
#if SD80BRIDGE_WINRT_MIDI_TEST

// ============================================================
// Windows.Devices.Midi / C++/WinRT MIDI Input Probe
//
// This is deliberately opt-in and independent of the existing
// USB-MIDI receive loop.
//
// First test routing:
//   Windows MIDI input
//        -> Part B
...
//        -> same MIDI channel
//
// Once this input path is proven, the next step can add a
// selectable Part A/B routing policy.
// ============================================================

struct WinRtMidiProbeContext
{
    winrt::Windows::Devices::Midi::MidiInPort midiIn{ nullptr };
    winrt::event_token messageReceivedToken{};
    WINUSB_INTERFACE_HANDLE winusbHandle = nullptr;
};

static std::shared_ptr<WinRtMidiProbeContext> g_winRtMidiContext;

static std::vector<BYTE> ReadWinRtMidiBytes(
    winrt::Windows::Devices::Midi::IMidiMessage const& message)
{
    const auto raw =
        message.RawData();
    ...
    auto reader =
        winrt::Windows::Storage::Streams::DataReader::FromBuffer(raw);
...
static bool ForwardWinRtMidiMessage(
    WINUSB_INTERFACE_HANDLE winusbHandle,
    winrt::Windows::Devices::Midi::IMidiMessage const& message)
{
    const std::vector<BYTE> bytes =
        ReadWinRtMidiBytes(message);
    ...
    const BYTE status = bytes[0];
    const BYTE data1 = bytes[1];
    const BYTE data2 = bytes[2];

    const BYTE type = status & 0xF0;

    // First probe: standard 3-byte Channel Voice messages.
    if ((status & 0x80) == 0 ||
        (type != 0x80 &&
            type != 0x90 &&
            type != 0xA0 &&
    ...
    // First probe: route Windows/DAW MIDI to Part B / Cable 1.
    return SendSD80Midi(
        winusbHandle,
        'B',
        channel,
...
static bool RunWinRtMidiInputProbe(
    WINUSB_INTERFACE_HANDLE winusbHandle)
{
    ...
    std::wcout
        << L"WINDOWS MIDI (WinRT) INPUT PROBE"
        << std::endl;
    ...
    std::wcout
        << L"[INFO ] MIDI input device count = "
        << devices.Size()
        << std::endl;
    ...
    {
        std::wcout
            << L"[ERROR] No Windows.Devices.Midi input devices found."
            << std::endl;
    ...
    const uint32_t index =
        SD80BRIDGE_WINRT_MIDI_DEVICE_INDEX;

    if (index >= devices.Size())
    {
        std::wcout
            << L"[ERROR] SD80BRIDGE_WINRT_MIDI_DEVICE_INDEX="
            << index
            << L" is out of range."
            << std::endl;
    ...
    g_winRtMidiContext =
        std::make_shared<WinRtMidiProbeContext>();

    g_winRtMidiContext->winusbHandle =
        winusbHandle;

    const hstring deviceId =
        devices.GetAt(index).Id();

    const auto openOperation =
        MidiInPort::FromIdAsync(
            deviceId);

    g_winRtMidiContext->midiIn =
        openOperation.get();

    if (!g_winRtMidiContext->midiIn)
    {
        std::wcout
            << L"[ERROR] MidiInPort::FromIdAsync returned null."
            << std::endl;

        g_winRtMidiContext.reset();
        return false;
    }

    g_winRtMidiContext->messageReceivedToken =
        g_winRtMidiContext->midiIn.MessageReceived(
            [](
                MidiInPort const&,
                MidiMessageReceivedEventArgs const& args)
            {
                try
                {
                    if (g_winRtMidiContext)
                    {
                        ForwardWinRtMidiMessage(
                            g_winRtMidiContext->winusbHandle,
                            args.Message());
                    }
                }
                ...
                {
                    std::wcerr
                        << L"[ERROR] WinRT MIDI callback HRESULT=0x"
                        << std::hex
    ...
    std::wcout
        << L"[INFO ] WinRT MIDI input opened."
        << std::endl;
    ...
    std::wcout
        << L"[INFO ] Windows MIDI input is routed to Part B / Cable 1."
        << std::endl;

    return true;
}

#endif // SD80BRIDGE_WINRT_MIDI_TEST
...
bool ProbeSD80WinUSB()
{
    ...
    std::wcout
        << L"SD80Bridge MIDI Runtime v10.16"
        << std::endl;
...
#if SD80BRIDGE_WINRT_MIDI_TEST
    if (!RunWinRtMidiInputProbe(
        winusbHandle))
    {
        std::wcout
            << L"[ERROR] WinRT MIDI input probe failed."
            << std::endl;
